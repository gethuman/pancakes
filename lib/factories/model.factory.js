 ...
 * @returns {{}}
 */
ModelFactory.prototype.getResources = function () {
    var resources = {};
    var me = this;
    ...
    _.each(names, function (name) {
        var modelName = utils.getPascalCase(name);
        var resourceName = utils.getCamelCase(name) + 'Resource';
        resources[modelName] = me.injector.loadModule(resourceName);
    });
...
ModelFactory.prototype.isCandidate = function (modelName) {
    return modelName && this.resources[modelName] ? true : false;
};
...
ModelFactory.prototype.getModel = function (service, mixins) {
    ...
    if (service.create && service.update) {
...
ModelFactory.prototype.create = function (modulePath, moduleStack) {
    ...
    // if model doesn't exist, then return null
    var resource = this.resources[modulePath];
    if (!resource) {
        return null;
    }
    ...
    return this.cache[modulePath] = this.getModel(service, resource.mixins);
};
