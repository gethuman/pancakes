 ...
 * @param debugParams
 */
ServiceFactory.prototype.addDebugEmitter = function (calls, eventNameBase, debugParams) {
...
ServiceFactory.prototype.putItAllTogether = function (serviceInfo, resource, adapter, filters) {
    ...
    for (i = 0; i < methods.length; i++) {
        ...
        this.addDebugEmitter(calls, eventNameBase, { type: 'service', timing: 'before' });
        ...
        _.each(filters.beforeFilters, function (filterInfo) {
            if (filterInfo[method] || filterInfo.all) {
                me.addDebugEmitter(calls, eventNameBase, { type: filterInfo.name, timing: 'before' });
                calls.push(filters[filterInfo.name].bind(filters));
                me.addDebugEmitter(calls, eventNameBase, { type: filterInfo.name, timing: 'after' });
            }
        });

        // this is the primary target for the service call; after this is the response going out
        this.addDebugEmitter(calls, eventNameBase, { type: 'adapter', timing: 'before' });
        calls.push(adapter[method].bind(adapter));
        this.addDebugEmitter(calls, eventNameBase, { type: 'adapter', timing: 'after' });
        ...
        this.addDebugEmitter(calls, eventNameBase, { type: 'service', timing: 'after' });
...
/**
 * We need to make sure that whenver a service method is called, an event is sent to the bus
 * after it is done with the results. This is key for the event propagation system. This
 * function will loop through all methods on the service and monkey patch them such that
 * after completition, the event is emitted. This will ensure that even if a service method
 * is called internally (i.e. from another service method) the event will be fired.
 *
 * @param serviceInfo
 * @param resource
 * @param adapter
 */
ServiceFactory.prototype.addMethodEmitters = function (serviceInfo, resource, adapter) {
    var adapterName = serviceInfo.adapterName;

    _.each(resource.methods[adapterName], function (methodName) {
        if (!adapter[methodName]) {
            return true;
        }

        // save the original method and then monkey patch it with a new function
        var orig = adapter[methodName].bind(adapter);
        adapter[methodName] = function (req) {
            ...
            orig(req)
                .then(function (res) {
                    ...
                    // before resolving back to the caller, send an event to the bus
                    var eventName = resource.name + '.' + adapterName + '.' + methodName;
                    var name = { resource: resource.name, adapter: adapterName, method: methodName };
                    eventBus.emit(eventName, { name: name, payload: res });
...
ServiceFactory.prototype.getAdapter = function (serviceInfo, resource, moduleStack) {
    ...
    this.addMethodEmitters(serviceInfo, resource, adapter);
    return adapter;
};
