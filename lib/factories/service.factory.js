 ...
 * @returns {Function}
 */
var initialRequestFilter = function (resource, method) {
    return function (req) {
        req = req || {};
 ...
 * @returns {Q}
 */
var initialResponseFilter = function (res) {
    if (!res || !res.data || !res.resource) {
        throw new Error('Adapter does not contain data and/or resource. In your adapter, ' +
 ...
 * @returns {Q}
 */
var finalResponseFilter = function (res) {
    res = res || {};
...
var putItAllTogether = function (serviceInfo, resource, adapter, filters) {
    ...
    for (i = 0; i < methods.length; i++) {
        ...
        calls.push(initialRequestFilter(resource, method));
        ...
        calls.push(initialResponseFilter);
        ...
        calls.push(finalResponseFilter);
 ...
 */
var getAdapter = function (serviceInfo, moduleStack, injector) {
    var servicesDir = injector.servicesDir;
    var servicesFullPath = injector.rootDir + '/' + servicesDir;
    ...
    // get the adapter
    var adapter = injector.loadModule(servicesDir + adapterPath, moduleStack);
    ...
    if (fs.existsSync(servicesFullPath + overridePath + '.js')) {
        ...
        _.extend(adapter, injector.loadModule(servicesDir + overridePath, moduleStack));
    }
...
var create = function (serviceName, moduleStack, injector) {
    ...
    var adapter = getAdapter(serviceInfo, moduleStack, injector);
