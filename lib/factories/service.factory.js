        ...
        adapter[method] = function (req) {
            ...
            return me.validateRequestParams(req, resource, method)
                ...
                .then(function (res) {
                    ...
                    // emit event after origin method called; this is what most reactors key off of
...
ServiceFactory.prototype.validateRequestParams = function (req, resource, method) {
    ...
    for(var i = 0; i < requiredParams.length; i++) {
        ...
        if (!req[param]) {
            deferred.reject(resource.name + ' ' + method + ' missing ' + param + '. Required params: ' + JSON.stringify(requiredParams));
    ...
    if (eitherorParams.length) {
        ...
        if (!eitherorExists) {
            deferred.reject(resource.name + ' ' + method + ' must have one of the following params: ' + JSON.stringify(eitherorParams));
            return deferred.promise;
        }
    }
    ...
    for (var key in req) {
        if (req.hasOwnProperty(key)) {
            if (validParams.indexOf(key) < 0) {
                deferred.reject('For ' + resource.name + ' ' + method + ' the key ' + key + ' is not allowed. Valid params: ' + JSON.stringify(validParams));
                return deferred.promise;
            }
        }
    }
