...
InternalObjectFactory.prototype.loadResources = function () {
    ...
    if (!fs.existsSync(p.normalize(resourcesDir))) {
        return resources;
    }

    var me = this;
    var resourceNames = fs.readdirSync(p.normalize(resourcesDir));
...
InternalObjectFactory.prototype.loadAdapters = function () {
    ...
    if (!fs.existsSync(p.normalize(adaptersDir))) {
        return adapters;
    }

    var me = this;
    var adapterNames = fs.readdirSync(p.normalize(adaptersDir));
...
InternalObjectFactory.prototype.loadReactors = function () {
    ...
    if (!fs.existsSync(p.normalize(reactorsDir))) {
        return reactors;
    }

    var me = this;
    var reactorNames = fs.readdirSync(p.normalize(reactorsDir));
