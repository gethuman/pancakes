 ...
 */
var fs =    require('fs');
var jeff =  require('jeff-jangular');
...
ViewFactory.prototype.getServerPartial = function (appName, dependencies, moduleStack) {
    ...
    function serverPartial (partialName, model, isAngular) {
        ...
        partial = fs.existsSync(nameInAppDir + '.js') ? require(nameInAppDir) : require(nameInCommonDir);
...
ViewFactory.prototype.create = function (modulePath, moduleStack, options) {
    ...
    return options.isAngular ?
        jeff.angularize(jeff.naked(injectedView), deps.model).toString() :
        jeff.naked(injectedView).toString();
};
