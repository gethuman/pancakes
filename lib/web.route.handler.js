...
var routeInfoCache = lruCache({ max: 100, maxAge: 3600000 }); // 1 hr
var pageCache = lruCache({ max: 1000, maxAge: 3600000 });     // 1 hr
var appRoutes = {};
...
function getRouteInfo(appName, urlRequest, query, lang) {
    ...
    var cachedRouteInfo = routeInfoCache.get(cacheKey);
    if (cachedRouteInfo) { return cachedRouteInfo; }
...
function processWebRequest(routeInfo, callbacks) {
    ...
    return getInitialModel(routeInfo, page)
        .then(function (model) {
            ...
            //console.log(routeInfo.url + ' initial model in ' + (endTime - startTime));
            ...
            // check to see if we should use the page cache
            var cacheKey = routeInfo.url + '||' + JSON.stringify(model);
            var cachedPage = pageCache.get(cacheKey);
            if (cachedPage) {
                return cachedPage;
            }
            ...
            //endTime = (new Date()).getTime();
            //console.log(routeInfo.url + ' serverPreprocessing in ' + (endTime - startTime));
            //startTime = (new Date()).getTime();
            //var ret =  clientPlugin.renderPage(routeInfo, page, model);
            //endTime = (new Date()).getTime();
            //console.log(routeInfo.url + ' renderPage in ' + (endTime - startTime));
            //return ret;

            // finally use the client side plugin to do the rendering
            var renderedPage = clientPlugin.renderPage(routeInfo, page, model);
            pageCache.set(cacheKey, renderedPage);
            return renderedPage;
