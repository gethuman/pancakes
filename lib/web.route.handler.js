    ...
    _.each(apps, function (app, appName) {
        _.each(app.routes, function (route) {
            ...
            if (route.inline && fs.existsSync(filePath)) {
                inlineCssCache[route.name] = fs.readFileSync(filePath, { encoding: 'utf8' });
            }
        });
    });
}
 ...
 */
function getRouteInfo(appName, urlRequest, query, lang, user, referer) {
...
function processWebRequest(routeInfo, callbacks) {
    ...
    return getInitialModel(routeInfo, page)
        ...
        .then(function (cachedPage) {
            ...
            return clientPlugin.renderPage(routeInfo, page, initialModel, inlineCssCache[routeInfo.name]);
        })
