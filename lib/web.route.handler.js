...
function convertUrlSegmentToRegex(segment) {
    if ( segment.indexOf('{') > -1 ) {  // if it has this format "{stuff}" - then it's regex-y
        var pieces = segment.split(':'); // this could be a named regex {id:[0-9]+} or just name {companySlug}
        if ( pieces.length === 2 ) {
            return pieces[1].substring(0, pieces[1].length - 1); // capture regex and trim trailing "}"
        } else if ( pieces.length === 1 ) {
            return '[a-zA-Z0-9\\-_~]+'; // capture... whatever
        } else {
...
function getTokenValuesFromUrl(pattern, url) {
    ...
    pattern.split('/').forEach(function(segment, idx) {
        if ( segment.indexOf('{') > -1 ) { // this has this form {stuff}, so let's dig
            segment = segment.substring(1, segment.length - 1); // peel off the { and } at the ends
            var pieces = segment.split(':');
            if ( pieces.length === 2 || pieces.length === 1 ) {
                tokenValues[pieces[0]] = urlSegments[idx];
            }
