...
var DependencyInjector = function (opts) {
    ...
    // clear cache of factories
    serviceFactory.clearCache();
    flapjackFactory.clearCache();
    ...
    this.aliases = _.extend({}, this.loadMappings(this.rootDir, preloadDirs), opts.aliases);
    ...
    _.each(this.adapterMap, function (impl, type) {
        var fullName = utils.getModuleName(impl + '.' + type + '.adapter');
        var shortName = utils.getPascalCase(type + '.adapter');
        if (me.aliases[fullName]) {
            me.aliases[shortName] = me.aliases[fullName];
        }
    });
};
    ...
    for (i = 0; i < paths.length; i++) {
        ...
        for (j = 0; j < fileNames.length; j++) {
            ...
            if (utils.isJavaScript(fileName)) {
                name = utils.getModuleName(fileName);
