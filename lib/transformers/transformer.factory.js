...
function generateTransformer(name, transformerFns, templateDir, pluginOptions) {
    ...
    transformer.setTemplate(templateDir, name);
...
function getTransformer(name, clientPlugin, pluginOptions) {
    ...
    var transformerFns = clientPlugin.transformerFns;
    var templateDir = clientPlugin.templateDir;

    // if no transformer, throw error
    if (!transformerFns[name]) { throw new Error('No transformer called ' + name); }

    // generate all the transformers
    _.each(transformerFns, function (transformerFns, name) {
        cache[name] = generateTransformer(name, transformerFns, templateDir, pluginOptions);
    });
